...
- Hugging Face Space exists for the demo, though the Space is currently failing to build
...
The Space is still listed at [African Wildlife Classifier](https://huggingface.co/spaces/vannputhika/african-wildlife-classifier), but it currently shows a build error, so the live demo is down. There is no public GitHub repository under Chinteyley for this project.